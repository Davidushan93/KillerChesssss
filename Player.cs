    ...
    public class Player : MonoBehaviourPunCallbacks, IPunObservable
    {
        ...
        private void Start()
        {
            ...
            if (photonView.IsMine)
            {
                ...
                photonView.RPC("SyncProfile", RpcTarget.All, Launcher.myProfile.username);//CIAONE, Launcher.myProfile.level, Launcher.myProfile.xp);
        ...
        private void Update()
        {
            ...
            //AGGIORNO I DANNI!!!! (E IL NUMERO DI AMMONIZIONI)
            RefreshHealthBar();
        ...
        void FixedUpdate()
        {
            ...
            //CASINO PER LA CAMERA. NON TOCCARE
            if (sliding)
            {
        ...
        [PunRPC]
        private void SyncProfile(string p_username)//CIAONE, int p_level, int p_xp)
        {
            playerProfile = new ProfileData(p_username);//CIAONE, p_level, p_xp);
        ...
        public void OnTriggerEnter (Collider fallout)
        {
            if (photonView.IsMine)
            {
                TakeDamage(99999, -1);
