...
namespace Com.Davidushan.KillerChess
{
    public class Weapon : MonoBehaviourPunCallbacks
    {
        ...
        private Image teschio; //2 AGGIUNGO TESCHIO PER KILLS
        private Image pene; //2 AGGIUNGO PENE PER DEATHS
        ...
        private void Start()
        {
            ...
            // hitmarkerImage.color = CLEARWHITE; MODIFICA (tolta trasparenza "CLEARWHITE") DEL MIRINO
            pene = GameObject.Find("HUD/Stats/Deaths/PENE").GetComponent<Image>(); //2
            teschio = GameObject.Find("HUD/Stats/Kills/TESCHIO").GetComponent<Image>(); //2
        ...
        void Update()
        {
            ...
            if(photonView.IsMine)
            {
                ...
                {
                //    hitmarkerImage.color = Color.Lerp(hitmarkerImage.color, CLEARWHITE, Time.deltaTime * 2f); MODIFICA (TOLTO RITORNO ALLA TRASPERENZA DEL MIRINO DOPO 2 SECONDI)
                }
            }
        }
        ...
        void Shoot ()
        {
            ...
            for (int i = 0; i < Mathf.Max(1, currentGunData.pellets); i++)
            {
                ...
                if (Physics.Raycast(t_spawn.position, t_bloom, out t_hit, 1000f, canBeShot))
                {
                    ...
                    if (photonView.IsMine)
                    {
                        //SPARO NON IN LOCALE, MA SUL NETWORK!!! RICORDATELO, ALTRIMENTI CIAO DANNI
                        if (t_hit.collider.gameObject.layer == 11)
                        {
                            ...
                            pene.color = Color.white; //2 MOSTRO PENE
                            teschio.color = Color.white; //2 MOSTRO TESCHIO
                        ...
                        if (t_hit.collider.gameObject.layer == 12)
                        {
                            ...
                            //MOSTRO MIRINO (NON FUNZIA PERCHÈ HO DISATTIVATO LA TRASPARENZA ALL'INIZIO, QUINDI IL MIRINO È FISSO SULLO SCHERMO SEMPRE
                            hitmarkerImage.color = Color.white;
                            pene.color = Color.white; //2 MOSTRO PENE
                            teschio.color = Color.white; //2 MOSTRO TESCHIO
                            sfx.PlayOneShot(hitmarkerSound);
                            hitmarkerWait = 1f; //MOSTRO PER UN SECONDO
                        }
                    }
                }
            }
            ...
            sfx.Play();
            Debug.Log("VAI COL SUONO " + currentGunData.gunshotSound.name);
